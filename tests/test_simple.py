        ...
        select t.id from jsondata t
         where t.id in (4,9,14,20)
           and exists (
                ...
                where t0.type > 0
                  and 'Evelyn Waugh' = t0.value COLLATE NOCASE
                  and t0.type > 0
               )
        """)
        eq_([row['id'] for row in result], [9])
...
class TestCreate(TestBookStore):
    ...
    def setup(self):
        self.dbpath = os.path.join(os.path.dirname(__file__), 'bookstore.new.db')
        logger.debug('dbpath = %s' % self.dbpath)
